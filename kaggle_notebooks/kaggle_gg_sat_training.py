...
# 2. Inner-loop Top-K PGD attack with dynamic randomized masking ($k \sim \text{Uniform}(0.3, 0.7)$).
# 3. Parameter optimization under mixed clean + sparse adversarial loss.
# 4. Save and load the robust checkpoint.
...
# %% [code]
# 2. Define Top-K PGD Attack for GG-SAT Inner Loop
...
# %% [code]
# 3. Load CIFAR-10 Data with Augmentation
...
# %% [code]
# 4. GG-SAT Training Parameters
epochs = 5  # Set to 5 for fast demo in Kaggle. Set to 50 or 100 for production.
lr = 0.1
weight_decay = 5e-4
k_min = 0.3
k_max = 0.7
beta = 0.5  # Weight factor for mixed training loss
pure = False # Mixed Training (Clean + Adv)
...
print(f"Epochs: {epochs} | LR: {lr} | k_range: [{k_min}, {k_max}] | Mixed beta: {beta}")
print("="*60)

# %% [code]
# 5. GG-SAT Training Loop
for epoch in range(epochs):
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        batch_k_ratio = random.uniform(k_min, k_max)
        
        # 1) Generate Sparse Adversarial Perturbations (Inner maximization - 5 steps for training efficiency)
        model.eval()
